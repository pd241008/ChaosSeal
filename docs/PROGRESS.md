...
- ~~`visible_pct` ≈ 0.7% is suspiciously low~~ **RESOLVED 2026-09-14**: geometry verified correct against the spherical-cap analytic expectation (h=550 km, elev≥10° → 1.696% of Earth's surface; measured 1.8–2.5% per satellite, in family given 53°-inclination ground-track sampling from a mid-latitude station). The low number is per-(satellite, time)-sample scale, not a coverage bug: constellation-level **any_visible ≈ 42–59%** and mean 0.42 sats in view over a 1200 s window. `LinkStats` now reports `any_visible_pct` + `mean_sats_in_view`; invariant enforced by `TestVisibilityPhysics`. Note: geometry is seed-independent (fixed phases), so visibility is deterministic across seeds; window length is the only driver of the fraction.
